...
void initialize_practical_06_scene(Viewer& viewer)
{
    ...
    glm::vec3 lightColor(255.0/255, 220.0/255, 248.0/255);
    DirectionalLightPtr directionalLight =
        std::make_shared<DirectionalLight>(lightDirection, lightColor, lightColor, lightColor);
...
void practical_06_movingCylinder(Viewer& viewer, const ShaderProgramPtr& shader)
{
    ...
    cylinder->addParentTransformKeyframe(1.5, GeometricTransformation(glm::vec3(0.0, 1.0, 1.0), glm::angleAxis((float)(2.0*M_PI/4.0), glm::vec3(0.0,1.0,0.0))) );
